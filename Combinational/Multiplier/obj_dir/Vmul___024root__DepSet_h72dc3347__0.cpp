// Verilated -*- C++ -*-
// DESCRIPTION: Verilator output: Design implementation internals
// See Vmul.h for the primary calling header

#include "Vmul__pch.h"
#include "Vmul___024root.h"

VL_ATTR_COLD void Vmul___024root___eval_initial__TOP(Vmul___024root* vlSelf);
VlCoroutine Vmul___024root___eval_initial__TOP__Vtiming__0(Vmul___024root* vlSelf);

void Vmul___024root___eval_initial(Vmul___024root* vlSelf) {
    (void)vlSelf;  // Prevent unused variable warning
    Vmul__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vmul___024root___eval_initial\n"); );
    auto &vlSelfRef = std::ref(*vlSelf).get();
    // Body
    Vmul___024root___eval_initial__TOP(vlSelf);
    Vmul___024root___eval_initial__TOP__Vtiming__0(vlSelf);
    vlSelfRef.__Vtrigprevexpr___TOP__mul_tb__DOT__a__0 
        = vlSelfRef.mul_tb__DOT__a;
    vlSelfRef.__Vtrigprevexpr___TOP__mul_tb__DOT__b__0 
        = vlSelfRef.mul_tb__DOT__b;
    vlSelfRef.__Vtrigprevexpr___TOP__mul_tb__DOT__p__0 
        = vlSelfRef.mul_tb__DOT__p;
}

void Vmul___024root___act_sequent__TOP__0(Vmul___024root* vlSelf);

void Vmul___024root___eval_act(Vmul___024root* vlSelf) {
    (void)vlSelf;  // Prevent unused variable warning
    Vmul__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vmul___024root___eval_act\n"); );
    auto &vlSelfRef = std::ref(*vlSelf).get();
    // Body
    if ((8ULL & vlSelfRef.__VactTriggered.word(0U))) {
        Vmul___024root___act_sequent__TOP__0(vlSelf);
    }
}

VL_INLINE_OPT void Vmul___024root___act_sequent__TOP__0(Vmul___024root* vlSelf) {
    (void)vlSelf;  // Prevent unused variable warning
    Vmul__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vmul___024root___act_sequent__TOP__0\n"); );
    auto &vlSelfRef = std::ref(*vlSelf).get();
    // Body
    vlSelfRef.mul_tb__DOT__p = ((IData)(vlSelfRef.mul_tb__DOT__a) 
                                * (IData)(vlSelfRef.mul_tb__DOT__b));
}

void Vmul___024root___nba_sequent__TOP__0(Vmul___024root* vlSelf);

void Vmul___024root___eval_nba(Vmul___024root* vlSelf) {
    (void)vlSelf;  // Prevent unused variable warning
    Vmul__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vmul___024root___eval_nba\n"); );
    auto &vlSelfRef = std::ref(*vlSelf).get();
    // Body
    if ((7ULL & vlSelfRef.__VnbaTriggered.word(0U))) {
        Vmul___024root___nba_sequent__TOP__0(vlSelf);
    }
}

void Vmul___024root___timing_resume(Vmul___024root* vlSelf) {
    (void)vlSelf;  // Prevent unused variable warning
    Vmul__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vmul___024root___timing_resume\n"); );
    auto &vlSelfRef = std::ref(*vlSelf).get();
    // Body
    if ((8ULL & vlSelfRef.__VactTriggered.word(0U))) {
        vlSelfRef.__VdlySched.resume();
    }
}

void Vmul___024root___eval_triggers__act(Vmul___024root* vlSelf);

bool Vmul___024root___eval_phase__act(Vmul___024root* vlSelf) {
    (void)vlSelf;  // Prevent unused variable warning
    Vmul__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vmul___024root___eval_phase__act\n"); );
    auto &vlSelfRef = std::ref(*vlSelf).get();
    // Init
    VlTriggerVec<4> __VpreTriggered;
    CData/*0:0*/ __VactExecute;
    // Body
    Vmul___024root___eval_triggers__act(vlSelf);
    __VactExecute = vlSelfRef.__VactTriggered.any();
    if (__VactExecute) {
        __VpreTriggered.andNot(vlSelfRef.__VactTriggered, vlSelfRef.__VnbaTriggered);
        vlSelfRef.__VnbaTriggered.thisOr(vlSelfRef.__VactTriggered);
        Vmul___024root___timing_resume(vlSelf);
        Vmul___024root___eval_act(vlSelf);
    }
    return (__VactExecute);
}

bool Vmul___024root___eval_phase__nba(Vmul___024root* vlSelf) {
    (void)vlSelf;  // Prevent unused variable warning
    Vmul__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vmul___024root___eval_phase__nba\n"); );
    auto &vlSelfRef = std::ref(*vlSelf).get();
    // Init
    CData/*0:0*/ __VnbaExecute;
    // Body
    __VnbaExecute = vlSelfRef.__VnbaTriggered.any();
    if (__VnbaExecute) {
        Vmul___024root___eval_nba(vlSelf);
        vlSelfRef.__VnbaTriggered.clear();
    }
    return (__VnbaExecute);
}

#ifdef VL_DEBUG
VL_ATTR_COLD void Vmul___024root___dump_triggers__nba(Vmul___024root* vlSelf);
#endif  // VL_DEBUG
#ifdef VL_DEBUG
VL_ATTR_COLD void Vmul___024root___dump_triggers__act(Vmul___024root* vlSelf);
#endif  // VL_DEBUG

void Vmul___024root___eval(Vmul___024root* vlSelf) {
    (void)vlSelf;  // Prevent unused variable warning
    Vmul__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vmul___024root___eval\n"); );
    auto &vlSelfRef = std::ref(*vlSelf).get();
    // Init
    IData/*31:0*/ __VnbaIterCount;
    CData/*0:0*/ __VnbaContinue;
    // Body
    __VnbaIterCount = 0U;
    __VnbaContinue = 1U;
    while (__VnbaContinue) {
        if (VL_UNLIKELY((0x64U < __VnbaIterCount))) {
#ifdef VL_DEBUG
            Vmul___024root___dump_triggers__nba(vlSelf);
#endif
            VL_FATAL_MT("mul_tb.v", 1, "", "NBA region did not converge.");
        }
        __VnbaIterCount = ((IData)(1U) + __VnbaIterCount);
        __VnbaContinue = 0U;
        vlSelfRef.__VactIterCount = 0U;
        vlSelfRef.__VactContinue = 1U;
        while (vlSelfRef.__VactContinue) {
            if (VL_UNLIKELY((0x64U < vlSelfRef.__VactIterCount))) {
#ifdef VL_DEBUG
                Vmul___024root___dump_triggers__act(vlSelf);
#endif
                VL_FATAL_MT("mul_tb.v", 1, "", "Active region did not converge.");
            }
            vlSelfRef.__VactIterCount = ((IData)(1U) 
                                         + vlSelfRef.__VactIterCount);
            vlSelfRef.__VactContinue = 0U;
            if (Vmul___024root___eval_phase__act(vlSelf)) {
                vlSelfRef.__VactContinue = 1U;
            }
        }
        if (Vmul___024root___eval_phase__nba(vlSelf)) {
            __VnbaContinue = 1U;
        }
    }
}

#ifdef VL_DEBUG
void Vmul___024root___eval_debug_assertions(Vmul___024root* vlSelf) {
    (void)vlSelf;  // Prevent unused variable warning
    Vmul__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vmul___024root___eval_debug_assertions\n"); );
    auto &vlSelfRef = std::ref(*vlSelf).get();
}
#endif  // VL_DEBUG
